...
namespace touchgfx
{
    ...
    MJPEGReader::MJPEGReader()
        : movieEndedAction(0), widget(0), frameBuffer(0), temporaryFrameBuffer(0), lineBuffer(0),
          fileBuffer(0), bufferStartOffset(0), dataLength(0),
          aviFile(0), lastError(AVI_NO_ERROR), fps(0),
          frameCounter(0), frameTick16frame(0), frameTickLost(0),
          msBetweenTicks(0), msTime(0), frameNumber(0),
          outputX(0), outputY(0), outputWidth(0), outputHeight(0), outputRect(),
          playing(false), repeating(false), headerRead(false),
          msPerFrame(0), numberOfFrames(0),
          frameWidth(0), frameHeight(0),
          firstFrameOffset(0), indexOffset(0)
    {
        useHWDecoding = true;
    }
    ...
    bool MJPEGReader::getRepeat(void)
    {
    	return repeating;
    }
    ...
    void MJPEGReader::showFirstFrame()
    {
        ...
        outputWidth = widget->getWidth();
        outputHeight = widget->getHeight();

        //center vertically
        outputY = (outputHeight - frameHeight)/2;
        //center horozontally
        outputX = (outputWidth - frameWidth)/2;
        ...
        if (lastError == AVI_NO_ERROR)
        {
            if (streamNo == STREAM0 && chunkType == TYPEDC)
            {
                ...
                if (show)
                {
                    ...
                    if (!decodeFrame(offset, playing ? (uint8_t*)framebuffer : frameBuffer))
                    {
...
//                    else
//                    {
//                      widget->invalidateRect(outputRect);
//                    }
                }
            }
            //forward to next frame
    ...
    void MJPEGReader::readData(uint32_t offset, uint32_t length)
    {
        ...
        uint32_t n = fileinput::readFile(aviFile, fileBuffer, length);
    ...
    bool MJPEGReader::decodeFrame(uint32_t offset, uint8_t* const buffer, int mode)
    {
#ifndef SIMULATOR      
      if (useHWDecoding && mode)
      {
        ...
        JPEG_Decode_DMA(&HwJPEG_Context, fileBuffer, dataLength, (uint32_t)temporaryFrameBuffer, (uint32_t)buffer);
      }
      else
#endif        
      {
