...
#define AUDIO_EOF_NOTIFICATION 0x01
#define STORAGE_USB_DISCONNECTED 0x02
#define STORAGE_USB_CONNECTED 0x03
#define STORAGE_MSD_DISCONNECTED 0x04
#define STORAGE_MSD_CONNECTED 0x05

#define GUI_SCREEN_A 0x06
#define GUI_SCREEN_B 0x07
#define GUI_SCREEN_C 0x08
#define GUI_SCREEN_D 0x09
#define GUI_SCREEN_E 0x0A
#define GUI_SCREEN_F 0x0B
#define GUI_SCREEN_G 0x0C
#define GUI_SCREEN_H 0x0D
#define GUI_SCREEN_I 0x0E
#define GUI_SCREEN_J 0x0F
#define GUI_SCREEN_K 0x10
#define GUI_SCREEN_L 0x11
...
//    }
    ...
    if (xQueueReceive(GUITaskQueue, &msg, 0) == pdPASS)
    {
        ...
        if (msg == GUI_SCREEN_A)
        {
            if (modelListener)
            {
            	debug(DEBUG_ENABLED, DEBUG_LEVEL_INFORMATION, "GUI_SCREEN_A\r\n");
            	const char video[] = "0://VIDEO/TROPIC.AVI";
            	modelListener->playVideoFile(video);
            }
        }
        /* GUI screen B */
        if (msg == GUI_SCREEN_B)
        {
            if (modelListener)
            {
            	debug(DEBUG_ENABLED, DEBUG_LEVEL_INFORMATION, "GUI_SCREEN_B\r\n");
            	const char video[] = "0://VIDEO/TROPIC.AVI";
            	modelListener->playVideoFile(video);
            }
        }
        ...
        if (msg == GUI_SCREEN_C)
        {
            if (modelListener)
            {
            	debug(DEBUG_ENABLED, DEBUG_LEVEL_INFORMATION, "GUI_SCREEN_C\r\n");
            	const char video[] = "0://VIDEO/OCEAN.AVI";
            	modelListener->playVideoFile(video);
            }
        }
        /* GUI screen D */
        if (msg == GUI_SCREEN_D)
        {
            if (modelListener)
            {
            	const char video[] = "0://VIDEO/STREAM.AVI";
            	modelListener->playVideoFile(video);
            }
        }
        /* GUI screen E */
        if (msg == GUI_SCREEN_E)
        {
            if (modelListener)
            {
            	const char video[] = "0://VIDEO/RAIN.AVI";
            	modelListener->playVideoFile(video);
            }
        }
        /* GUI screen F */
        ...
        if(msg == 0x12)
        {
        	const char audio[] = "0://AUDIO/STARTUP.WAV";
//        	AUDIOPLAYER_SelectFile((uint8_t*)audio);
