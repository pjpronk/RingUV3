...
void rtcInitialization(void)
{
	...
	{
		writeDebug("HAL_RCC_OscConfig(&rcc_osc_init_struct) != HAL_OK", true);
	}
	...
	{
        writeDebug("HAL_RCCEx_PeriphCLKConfig(&periph_clk_init_struct) != HAL_OK", true);
	}
	...
	{
        writeDebug("HAL_RTC_Init(&rtc_handle) != HAL_OK", true);
	}
